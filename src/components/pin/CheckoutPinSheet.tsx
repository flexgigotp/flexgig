...
export default function CheckoutPinSheet({
  ...
  const [forgotStage, setForgotStage] = useState<ForgotStage>('none')
    ...
    // Block a pending app update from reloading the page while the user
  // has this sheet open — PIN digits, OTP entry, or new-PIN setup.
  useUpdateGuard()
...
const handleDigit = useCallback(
  (d: string) => {
    ...
    onSubmit(next).then((result) => {
      ...
      // On success the hook navigates away — the sheet unmounts,
      // no state cleanup needed here.
    })
  },
  ...
  usePinKeyboard(
    handleDigit,
    handleDelete,
    submitting || forgotStage !== 'none'
  )

  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      if (e.key === 'Escape' && !submitting && forgotStage === 'none') {
        onClose()
      }
    }
    window.addEventListener('keydown', handler)
    return () => window.removeEventListener('keydown', handler)
  }, [onClose, submitting, forgotStage])
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (window.history.state?.__fgCheckoutForgotBlock) {
        window.history.back()
      }
    }
  }, [blockingForgot])
  ...
  const showBio = biometricEnabled && typeof onBiometric === 'function'
      ...
      <div className="fg-cpin-sheet" role="dialog" aria-modal="true">
        ...
        <div className="fg-cpin-keypad">
          ...
          {showBio ? (
            <button
              type="button"
              className="fg-cpin-key fg-cpin-key-bio"
        ...
        {(submitting || biometricBusy) && <Loader transparent />}
      </div>
    </div>,
    document.body
  )
